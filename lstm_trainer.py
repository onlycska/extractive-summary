...
class Trainer:
    ...
    def fit(self, model: RecurrentClassifier, train_loader: DataLoader, val_loader: DataLoader,
            save_epochs: bool = True) -> RecurrentClassifier:
        """
        ...
        :param save_epochs: Whether to save the model after each epoch?
        ...
        for epoch in range(1, self.n_epochs + 1):
            ...
            if save_epochs:
                saving_path = os.path.join(os.getcwd(), 'models', 'lstm_model_epoch{epoch}.cpkt')
    ...
    def _val_epoch(self, val_loader: DataLoader) -> dict:
        ...
        if self.verbose:
            val_loader.set_description(f"Loss={loss:.3}; Acc:{acc:.4}")
    ...
    def load(cls, path: str):
        ...
        keys = ["config", "trainer_config", "emb_matrix", "state_dict"]
        for key in keys:
            if key not in ckpt:
                raise RuntimeError(f"Missing key {key} in checkpoint")
        new_model = RecurrentClassifier(ckpt["config"], ckpt["emb_matrix"])
